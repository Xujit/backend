...
import com.codingaxis.hr.config.Constants;
import com.codingaxis.hr.domain.User;
import com.codingaxis.hr.security.AuthoritiesConstants;
import com.codingaxis.hr.security.RandomUtil;
import com.codingaxis.hr.service.dto.PasswordChangeDTO;
import com.codingaxis.hr.service.dto.UserDTO;
import com.codingaxis.hr.web.rest.vm.KeyAndPasswordVM;
...
public class AccountResourceTest {
  ...
  private String authenticateUser(String username, String password) {
    ...
    var token = given().body(login).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
        .header(HttpHeaders.ACCEPT, MediaType.APPLICATION_JSON).when().post("/api/authenticate").then()
        .statusCode(OK.getStatusCode()).body("id_token", instanceOf(String.class)).body("id_token", notNullValue())
        .header(HttpHeaders.AUTHORIZATION, not(blankOrNullString())).extract().path("id_token");
  ...
  public void testNonAuthenticatedUser() {
    ...
    given().accept(MediaType.TEXT_PLAIN).get("/api/authenticate").then().statusCode(OK.getStatusCode())
        .body(is(emptyString()));
  }
  ...
  public void testAuthenticatedUser() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).accept(MediaType.TEXT_PLAIN).get("/api/authenticate")
        .then().statusCode(OK.getStatusCode()).body(containsString("test"));
  }
  ...
  public void testGetExistingAccount() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .get("/api/account").then().statusCode(OK.getStatusCode()).contentType(APPLICATION_JSON)
        .body("login", is("test")).body("firstName", is(user.firstName)).body("lastName", is(user.lastName))
        .body("email", is(user.email)).body("imageUrl", is(user.imageUrl)).body("langKey", is(user.langKey))
  ...
  public void testGetUnknownAccount() {
    ...
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).get("/api/account").then()
  ...
  public void testRegisterInvalidLogin() {
    ...
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).body(invalidUser).when().post("/api/register").then()
  ...
  public void testRegisterDuplicateLogin() {
    ...
    // First user
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).body(firstUser).when().post("/api/register").then()
        .statusCode(CREATED.getStatusCode());

    // Second (non activated) user
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).body(secondUser).when().post("/api/register").then()
        .statusCode(CREATED.getStatusCode());
  ...
  public void testRegisterDuplicateEmail() {
    ...
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).body(userWithUpperCaseEmail).when()
        .post("/api/register").then().statusCode(CREATED.getStatusCode());

    var testUser = get("/api/users/{login}", userWithUpperCaseEmail.login).then().statusCode(OK.getStatusCode())
        .extract().as(User.class);

    Assertions.assertEquals(testUser.email, "test-register-duplicate-email@example.com");
  ...
  public void testActivateAccount() throws Exception {
    ...
    var activatedUser = get("/api/users/{login}", user.login).then().statusCode(OK.getStatusCode()).extract()
        .as(User.class);

    Assertions.assertTrue(activatedUser.activated);
  }
  ...
  public void testActivateAccountWithWrongKey() throws Exception {
    ...
    // Activating user
    given().when().get("/api/activate?key={key}", "wrongActivationKey").then()
  ...
  public void testSaveAccount() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .body(userDTO).post("/api/account").then().statusCode(OK.getStatusCode());

    var updatedUser = get("/api/users/{login}", user.login).then().statusCode(OK.getStatusCode()).extract()
        .as(User.class);

    Assertions.assertEquals(updatedUser.firstName, userDTO.firstName);
     ...
     * assertThat(updatedUser.imageUrl).isEqualTo(userDTO.imageUrl); assertThat(updatedUser.activated).isEqualTo(true);
  ...
  public void testSaveInvalidEmail() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .body(userDTO).post("/api/account").then().statusCode(BAD_REQUEST.getStatusCode());
  ...
  public void testChangePasswordWrongExistingPassword() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .body(passwordChangeDTO).post("/api/account/change-password").then().statusCode(BAD_REQUEST.getStatusCode());
  }
  ...
  public void testChangePassword() throws Exception {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .body(passwordChangeDTO).post("/api/account/change-password").then().statusCode(OK.getStatusCode());
  }
  ...
  public void testRequestPasswordReset() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(TEXT_PLAIN)
        .body("password-reset@example.com").post("/api/account/reset-password/init").then()
  ...
  public void testRequestPasswordResetWrongEmail() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(TEXT_PLAIN)
        .body("password-reset-wrong-email@example.com").post("/api/account/reset-password/init").then()
        .statusCode(BAD_REQUEST.getStatusCode());
  }
  ...
  public void testFinishPasswordReset() {
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(TEXT_PLAIN).body(user.email)
        .post("/api/account/reset-password/init").then().statusCode(OK.getStatusCode());

    List<Mail> sent = this.mailbox.getMessagesSentTo(user.email);
    ...
    given().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).contentType(APPLICATION_JSON).accept(APPLICATION_JSON)
        .body(keyAndPassword).post("/api/account/reset-password/finish").then().statusCode(OK.getStatusCode());
  }
  ...
  public void testFinishPasswordResetWrongKey() throws Exception {
    ...
    given().contentType(APPLICATION_JSON).accept(APPLICATION_JSON).body(keyAndPassword).when()
        .post("/api/account/reset-password/finish").then().statusCode(INTERNAL_SERVER_ERROR.getStatusCode());
  }
}
